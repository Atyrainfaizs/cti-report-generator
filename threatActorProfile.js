document.getElementById('threatActorForm').addEventListener('submit', function(event) {
    ...
    const ttt = document.getElementById('ttt').value;
    ...
    // Collect IOC data
    const iocEntries = [];
    const iocForms = document.querySelectorAll('.ioc-entry');
    iocForms.forEach((form) => {
        const iocType = form.querySelector('h3').textContent.trim(); // Get IOC type from the heading
        let iocData;
        ...
        switch (iocType) {
            case 'Malware':
                iocData = {
                    type: 'Malware',
                    attribution: form.querySelector('input[name="attribution"]').value,
                    toolName: form.querySelector('input[name="toolName"]').value,
                    analysisReport: form.querySelector('textarea[name="analysisReport"]').value,
                    hashType: form.querySelector('select[name="hashType"]').value,
                    fileHash: form.querySelector('input[name="fileHash"]').value,
                    description: form.querySelector('textarea[name="description"]').value,
                    firstReported: form.querySelector('input[name="firstReported"]').value,
                    lastReported: form.querySelector('input[name="lastReported"]').value
                };
                break;
            ...
            case 'Network Indicator':
                iocData = {
                    ...
                    attribution: form.querySelector('input[name="attribution"]').value,
                    intrusionPhase: form.querySelector('select[name="intrusionPhase"]').value,
                    artifact: form.querySelector('input[name="artifact"]').value,
            ...
            case 'System Artifact':
                iocData = {
                    ...
                    attribution: form.querySelector('input[name="attribution"]').value,
                    tactic: form.querySelector('select[name="tactic"]').value,
                    hostArtifact: form.querySelector('input[name="hostArtifact"]').value,
            ...
            case 'CVE':
                iocData = {
                    type: 'CVE',
                    attribution: form.querySelector('input[name="attribution"]').value,
                    cveNumber: form.querySelector('input[name="cveNumber"]').value,
                    cvssScore: form.querySelector('input[name="cvssScore"]').value,
                    patchAvailable: form.querySelector('select[name="patchAvailable"]').value,
                    patchApplied: form.querySelector('select[name="patchApplied"]').value,
                    dateReported: form.querySelector('input[name="dateReported"]').value
                };
                break;

            default:
                console.error('Unknown IOC type:', iocType);
                return;
        }

        iocEntries.push(iocData);
    });
    ...
    const report = { 
        ...
        ttt,
        infrastructure,
        timeline: timelineEntries, // Add timeline data
        mitre: mitreEntries,
    ...
    saveReport(report);
    alert('Threat Actor Profile Report Saved!');
...
// Add IOC Form
const iocContainer = document.getElementById("iocContainer");
const addIocButton = document.getElementById("addIoc");
const iocTypeSelector = document.getElementById("iocTypeSelector");

// Function to create Malware IOC form
function createMalwareForm() {
    const malwareForm = document.createElement("div");
    malwareForm.classList.add("ioc-entry");
    malwareForm.innerHTML = `
        <h3>Malware</h3>
        <label for="attribution">Attribution:</label>
        <input type="text" name="attribution" required><br>
        <label for="toolName">Malware Tool Name:</label>
        <input type="text" name="toolName" required><br>
        <label for="analysisReport">Malware Analysis Report:</label>
        <textarea name="analysisReport" required></textarea><br>
        <label for="hashType">Hash Type:</label>
        <select name="hashType" required>
            <option value="">Select Hash Type</option>
            <option value="MD5">MD5</option>
            <option value="SHA1">SHA1</option>
            <option value="SHA256">SHA256</option>
        </select><br>
        <label for="fileHash">Associated File Hash:</label>
        <input type="text" name="fileHash" required><br>
        <label for="description">Brief Description:</label>
        <textarea name="description" required></textarea><br>
        <label for="firstReported">First Reported:</label>
        <input type="date" name="firstReported" required><br>
        <label for="lastReported">Last Reported:</label>
        <input type="date" name="lastReported" required><br>
        <button type="button" class="removeIoc">Remove</button>
        <hr>
    `;
    return malwareForm;
}

// Function to create Network Indicator IOC form
function createNetworkIndicatorForm() {
    const networkIndicatorForm = document.createElement("div");
    networkIndicatorForm.classList.add("ioc-entry");
    networkIndicatorForm.innerHTML = `
        <h3>Network Indicator</h3>
        <label for="attribution">Attribution:</label>
        <input type="text" name="attribution" required><br>
        <label for="intrusionPhase">Intrusion Phase:</label>
        <select name="intrusionPhase" required>
            <option value="">Select Intrusion Phase</option>
            <option value="Initial Access">Initial Access</option>
            <option value="Command and Control">Command and Control</option>
            <option value="Exfiltration">Delivery</option>
        </select><br>
        <label for="artifact">Network Artifact:</label>
        <input type="text" name="artifact" required><br>
        ...
        <hr>
    `;
    return networkIndicatorForm;
}

// Function to create System Artifact IOC form
function createSystemArtifactForm() {
    const systemArtifactForm = document.createElement("div");
    systemArtifactForm.classList.add("ioc-entry");
    systemArtifactForm.innerHTML = `
        <h3>System Artifact</h3>
        <label for="attribution">Attribution:</label>
        <input type="text" name="attribution" required><br>
        <label for="tactic">Tactic:</label>
        <select name="tactic" required>
        ...
        <label for="hostArtifact">Host Artifact:</label>
        <input type="text" name="hostArtifact" required><br>
        ...
        <hr>
    `;
    return systemArtifactForm;
}

// Function to create CVE IOC form
function createCveForm() {
    const cveForm = document.createElement("div");
    cveForm.classList.add("ioc-entry");
    cveForm.innerHTML = `
        <h3>CVE</h3>
        <label for="attribution">Attribution:</label>
        <input type="text" name="attribution" required><br>
        <label for="cveNumber">CVE Number:</label>
        <input type="text" name="cveNumber" required><br>
        <label for="cvssScore">CVSS Score:</label>
        <input type="number" name="cvssScore" min="0" max="10" step="0.1" required><br>
        <label for="patchAvailable">Patch Available:</label>
        <select name="patchAvailable" required>
            <option value="">Select Option</option>
            <option value="Yes">Yes</option>
            <option value="No">No</option>
        </select><br>
        <label for="patchApplied">Patch Applied:</label>
        <select name="patchApplied" required>
            <option value="">Select Option</option>
            <option value="Yes">Yes</option>
            <option value="No">No</option>
        ...
        <input type="date" name="dateReported" required><br>
        <button type="button" class="removeIoc">Remove</button>
        <hr>
    `;
    return cveForm;
...
// Add IOC button click event
addIocButton.addEventListener("click", () => {
    const selectedIocType = iocTypeSelector.value;

    let iocForm;
    switch (selectedIocType) {
        case "malware":
            iocForm = createMalwareForm();
            break;
        case "networkIndicator":
            iocForm = createNetworkIndicatorForm();
            break;
        case "systemArtifact":
            iocForm = createSystemArtifactForm();
            break;
        case "cve":
            iocForm = createCveForm();
            break;
        default:
    ...
    iocContainer.appendChild(iocForm);

    // Add remove functionality
    iocForm.querySelector(".removeIoc").addEventListener("click", () => {
        iocContainer.removeChild(iocForm);
    });
});
